...
if (!$data) {
    ...
    \Bitrix\Main\Diag\Debug::writeToFile($source, 'Invalid JSON', 'avs_booking.log');
    die('Invalid JSON');
}
...
switch ($legalEntity) {
    ...
    default:
        if (isset($data['object']['id'])) {
            ...
            if (!empty($orders)) $secretKey = Option::get('avs_booking', $orders[0]['LEGAL_ENTITY'] . '_secret_key', '');
        }
}
$signatureHeader = $_SERVER['HTTP_YOKASSA_SIGNATURE'] ?? '';
$signatureValid = false;
if ($secretKey && !empty($signatureHeader)) $signatureValid = verifyYookassaSignature($source, $signatureHeader, $secretKey);
if (!$signatureValid) {
    \Bitrix\Main\Diag\Debug::writeToFile(['ip' => $_SERVER['REMOTE_ADDR'], 'signature_header' => $signatureHeader, 'legal_entity' => $legalEntity, 'has_secret_key' => !empty($secretKey)], 'Yookassa webhook SIGNATURE verification FAILED', 'avs_booking.log');
...
$ipValid = false;
foreach ($allowedIps as $ipRange) if (ipInRange($clientIp, $ipRange)) {
    $ipValid = true;
    break;
}
if (!$ipValid) {
    \Bitrix\Main\Diag\Debug::writeToFile(['ip' => $clientIp, 'legal_entity' => $legalEntity], 'Yookassa webhook: IP not in allowed range, but signature valid', 'avs_booking.log');
}
try {
