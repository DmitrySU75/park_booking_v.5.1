...
class Api
{
    ...
    private function createOrder()
    {
        ...
        foreach ($required as $field) if (empty($data[$field])) {
    ...
    private function updateStatus()
    {
        ...
        $order = $orderId ? Order::get($orderId) : Order::getByOrderNumber($orderNumber);
        if (!$order) {
    ...
    private function updateOrder()
    {
        ...
        if (isset($data['new_start_time']) && isset($data['new_end_time'])) {
            ...
            if ($gazebo && $gazebo['resource_id']) {
                if (!$this->checkAvailability($gazebo['resource_id'], $newStart, $newEnd, $order['LIBREBOOKING_RESERVATION_ID'])) {
            ...
            $changes['time'] = ['old_start' => $order['START_TIME']->toString(), 'old_end' => $order['END_TIME']->toString(), 'new_start' => $newStart, 'new_end' => $newEnd];
        }
        if (isset($data['new_pavilion_id'])) {
            ...
            if ($newGazebo['resource_id']) {
                if (!$this->checkAvailability($newGazebo['resource_id'], $currentStart, $currentEnd, null)) {
            ...
            $changes['pavilion'] = ['old_id' => $order['PAVILION_ID'], 'old_name' => $order['PAVILION_NAME'], 'new_id' => $newPavilionId, 'new_name' => $newGazebo['name']];
        }
        if (empty($updateData)) {
    ...
    private function getOrders()
    {
        ...
        $this->successResponse(['orders' => $result, 'total' => count($result), 'period' => ['start' => $startDate, 'end' => $endDate]]);
    }
    ...
    private function updatePrices()
    {
        ...
        foreach ($prices as $priceItem) {
            ...
            if ($result) {
                $updated[] = ['pavilion_id' => $pavilionId, 'pavilion_name' => $gazebo['name'], 'effective_from' => $effectiveFrom, 'new_prices' => $updateData];
            } else {
                $errors[] = "Failed to update prices for pavilion ID {$pavilionId}";
            }
        }
        $this->successResponse(['effective_from' => $effectiveFrom, 'updated' => $updated, 'updated_count' => count($updated), 'errors' => $errors]);
    }
    ...
    private function savePriceHistory($pavilionId, $prices, $effectiveFrom)
    {
        ...
        $sql = "INSERT INTO avs_booking_price_history 
                ...
                VALUES ({$pavilionId}, " . ($fields['PRICE_HOUR'] ?? 'NULL') . ", " . ($fields['PRICE_DAY'] ?? 'NULL') . ", " . ($fields['PRICE_NIGHT'] ?? 'NULL') . ", '{$effectiveFrom}', NOW())";
        $DB->Query($sql);
    }
