...
/**
 * Файл: admin/avs_booking_dashboard.php
...
$sourceStats = ['site' => 0, '1c' => 0, 'manual' => 0, 'librebooking' => 0];
foreach ($weekOrders as $order) {
    if (strpos($order['ORDER_NUMBER'], 'ORD-') === 0) $sourceStats['site']++;
    elseif (strpos($order['ORDER_NUMBER'], '1C-') === 0) $sourceStats['1c']++;
    elseif (strpos($order['ORDER_NUMBER'], 'MAN-') === 0) $sourceStats['manual']++;
}
...
try {
    ...
    file_put_contents($logFile, date('Y-m-d H:i:s') . " [dashboard] API client created\n", FILE_APPEND);
...
<div class="dashboard-stats">
    <div class="stat-card">
        <h3>📱 Через сайт</h3>
        <div class="value"><?= $sourceStats['site'] ?></div>
        <div class="unit">броней за неделю</div>
    </div>
    <div class="stat-card">
        <h3>🖥️ Из 1С</h3>
        <div class="value"><?= $sourceStats['1c'] ?></div>
        <div class="unit">броней за неделю</div>
    </div>
    <div class="stat-card">
        <h3>📅 В LibreBooking</h3>
        <div class="value"><?= $sourceStats['librebooking'] ?></div>
        <div class="unit">броней сегодня</div>
    </div>
</div>
            ...
            <table class="order-table">
                ...
                <tbody>
                    <?php foreach ($libreBookingsToday as $booking): ?>
                        <tr>
                            <td><?= htmlspecialcharsbx($booking['pavilion_name']) ?></td>
                            <td><span class="status-badge status-paid">Занято</span></td>
                            <td><?= htmlspecialcharsbx($booking['source']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <table class="order-table">
            ...
            <tbody>
                <?php foreach ($libreBookings as $booking): ?>
                    <tr>
                        <td><?= htmlspecialcharsbx($booking['pavilion_name']) ?></td>
                        <td><?php if ($booking['available_today']): ?><span class="status-badge status-completed">🟢 Свободна</span><?php else: ?><span class="status-badge status-paid">🔴 Занята</span><?php endif; ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
