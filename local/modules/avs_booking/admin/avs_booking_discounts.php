...
/**
 * Файл: admin/avs_booking_discounts.php
...
<form method="post">
    ...
    <div class="form-row"><label>Код промокода:</label><input type="text" name="code" required style="text-transform:uppercase"></div>
    <div class="form-row"><label>Название:</label><input type="text" name="name" required size="40"></div>
    <div class="form-row"><label>Тип скидки:</label><select name="discount_type" required>
            <option value="percent">Процент (%)</option>
            <option value="fixed">Фиксированная сумма (руб)</option>
        </select></div>
    <div class="form-row"><label>Значение скидки:</label><input type="number" name="discount_value" step="0.01" required></div>
    <div class="form-row"><label>Действителен с:</label><input type="datetime-local" name="valid_from"></div>
    <div class="form-row"><label>Действителен до:</label><input type="datetime-local" name="valid_to"></div>
    <div class="form-row"><label>Мин. сумма заказа:</label><input type="number" name="min_order_amount" step="0.01"></div>
    <div class="form-row"><label>Макс. кол-во использований:</label><input type="number" name="max_uses"></div>
    <div class="form-row"><label>Активен:</label><select name="active">
    ...
    <tbody>
        <?php foreach ($discounts as $d): ?>
            <tr>
                ...
                <td><?php if ($d['VALID_FROM']) echo "с " . date('d.m.Y H:i', strtotime($d['VALID_FROM'])) . "<br>";
                    if ($d['VALID_TO']) echo "по " . date('d.m.Y H:i', strtotime($d['VALID_TO']));
                    if (!$d['VALID_FROM'] && !$d['VALID_TO']) echo "без ограничений"; ?></td>
                <td><?= $d['USES_COUNT'] ?><?= $d['MAX_USES'] ? " / " . $d['MAX_USES'] : '' ?></td>
            </tr>
        <?php endforeach; ?>
        <?php if (empty($discounts)): ?><tr>
