...
/**
 * Файл: /local/modules/avs_booking/admin/dashboard.php
...
<div class="sync-panel">
    ...
    <button onclick="quickSync()" class="adm-btn" id="syncBtn" style="margin-left: 10px;">
        📅 За последние 24 часа
    </button>
    <button onclick="fullSync()" class="adm-btn" id="fullSyncBtn">
        📆 За 30 дней
    </button>
...
<script>
    function quickSync() {
        ...
        fetch('/local/modules/avs_booking/ajax_sync.php', {
                ...
                body: 'action=quick&sessid=' + BX.bitrix_sessid()
            })
    ...
    function fullSync() {
        ...
        fetch('/local/modules/avs_booking/ajax_sync.php', {
                ...
                body: 'action=full&sessid=' + BX.bitrix_sessid()
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
...
<?php if ($libreBookingError): ?>
    <div class="warning">
        ⚠️ Не удалось подключиться к LibreBooking API: <?= htmlspecialcharsbx($libreBookingError) ?>
    </div>
<?php endif; ?>
            ...
            <tbody>
                <?php foreach ($libreBookings as $booking): ?>
                    <tr>
                        <td><?= htmlspecialcharsbx($booking['pavilion_name']) ?></td>
                        <td>
                            <?php if ($booking['available_today']): ?>
                                <span class="status-badge status-completed">🟢 Свободна</span>
                            <?php else: ?>
                                <span class="status-badge status-paid">🔴 Занята</span>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
