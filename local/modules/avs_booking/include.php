...
require_once __DIR__ . '/lib/External/LibreBookingAPI.php';

if (!class_exists('LibreBookingAPI')) {
    class_alias('AVS\Booking\External\LibreBookingAPI', 'LibreBookingAPI');
}
...
class AVSBookingModule
{
    ...
    public static function getGazeboData($elementId)
    {
        ...
        $res = CIBlockElement::GetList(
            ...
            false,
            false,
            ['ID', 'NAME', 'PROPERTY_LIBREBOOKING_RESOURCE_ID', 'PROPERTY_PRICE_HOUR', 'PROPERTY_PRICE', 'PROPERTY_PRICE_NIGHT', 'PROPERTY_DEPOSIT_AMOUNT']
        );
        if ($element = $res->Fetch()) {
            $deposit = (float)$element['PROPERTY_DEPOSIT_AMOUNT_VALUE'];
            if (!$deposit) $deposit = self::getDefaultDeposit((int)$element['ID']);
            return [
                'id' => (int)$element['ID'],
    ...
    public static function isSummerPeriod($pavilionId, $date)
    {
        ...
        if (in_array($gazebo['name'], $noSummerParks)) return false;
    ...
    public static function getAvailableRentalTypes($pavilionId, $date)
    {
        ...
        foreach ($allTypes as $key => $type) {
            if ($type['price'] <= 0) unset($allTypes[$key]);
        }
        return $allTypes;
    }
    ...
    public static function calculateTimeRange($rentalType, $date, $pavilionId, $startHour = null, $hours = null)
    {
        ...
        switch ($rentalType) {
            ...
            case 'hourly':
                ...
                if ($endHour > $workEndHour) return null;
                return [
                    'start' => $start,
